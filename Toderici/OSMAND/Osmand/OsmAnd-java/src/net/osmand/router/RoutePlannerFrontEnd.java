...
public class RoutePlannerFrontEnd {
	...
	/*
	 this method was modified and added and extra parameter, that represents the file in which the results will be printed
	 */
	public List<RouteSegmentResult> searchRoute(final RoutingContext ctx, LatLon start, LatLon end, List<LatLon> intermediates, TrafficDataSet traffic, long delay) throws IOException, InterruptedException {
		return searchRoute(ctx, start, end, intermediates, null, traffic, delay);
	}
	...
	public List<RouteSegmentResult> searchRoute(final RoutingContext ctx, LatLon start, LatLon end, List<LatLon> intermediates,
	                                            PrecalculatedRouteDirection routeDirection,TrafficDataSet traffic, long delay) throws IOException, InterruptedException {
		...
		if (ctx.calculationMode == RouteCalculationMode.COMPLEX && routeDirection == null
			...
			List<RouteSegmentResult> ls = searchRoute(nctx, start, end, intermediates, traffic, delay);
		...
		List<RouteSegmentResult> res = searchRoute(ctx, points, routeDirection, traffic, delay);
		// make start and end more precise
		makeStartEndPointsPrecise(res, start, end, intermediates);

		//Commented not to always print the routes
		/*
		if (res != null) {
			new RouteResultPreparation().printResults(ctx, start, end, res);
		}
	...
	private List<RouteSegmentResult> searchRouteInternalPrepare(final RoutingContext ctx, RouteSegmentPoint start, RouteSegmentPoint end,
		...
		if (ctx.nativeLib != null) {
			...
			if (useOldVersion) {
				new BinaryRoutePlannerOld().searchRouteInternal(ctx, start, end, traffic, delay);
			} else {
				ctx.finalRouteSegment = new BinaryRoutePlanner().searchRouteInternal(ctx, start, end, recalculationEnd, traffic, delay);
			}
			// 4. Route is found : collect all segments and prepare result
			return new RouteResultPreparation().prepareResult(ctx, ctx.finalRouteSegment,traffic ,delay);
		}
	}
	...
	private List<RouteSegmentResult> runNativeRouting(final RoutingContext ctx, RouteSegment recalculationEnd) throws IOException {
		...
		return new RouteResultPreparation().prepareResult(ctx, result, new TrafficDataSet());
	}


	private List<RouteSegmentResult> searchRoute(final RoutingContext ctx, List<RouteSegmentPoint> points, PrecalculatedRouteDirection routeDirection, TrafficDataSet traffic, long delay)
			throws IOException, InterruptedException {
		if (points.size() <= 2) {
			if (!useSmartRouteRecalculation) {
				ctx.previouslyCalculatedRoute = null;
			}
			return searchRoute(ctx, points.get(0), points.get(1), routeDirection, traffic, delay);
		}
		...
		for (int i = 0; i < points.size() - 1; i++) {
			...
			List<RouteSegmentResult> res = searchRouteInternalPrepare(local, points.get(i), points.get(i + 1), routeDirection, traffic, delay);
	...
	private List<RouteSegmentResult> searchRoute(final RoutingContext ctx, RouteSegmentPoint start, RouteSegmentPoint end,
		...
		List<RouteSegmentResult> result = searchRouteInternalPrepare(ctx, start, end, routeDirection, traffic, delay);
