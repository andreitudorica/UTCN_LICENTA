...
public class RouteResultPreparation {
	 ...
	 */
	List<RouteSegmentResult> prepareResult(RoutingContext ctx, FinalRouteSegment finalSegment, TrafficDataSet traffic, long delay) throws IOException {
		List<RouteSegmentResult> result  = convertFinalSegmentToResults(ctx, finalSegment);
		prepareResult(ctx, result, traffic);

		updateTraffic(ctx,result, traffic, delay);

		return result;
	}

	private void updateTraffic(RoutingContext ctx,List<RouteSegmentResult> results, TrafficDataSet traffic, long delay) {

		double checkpointTime = 0;

		//System.out.println("UPDATE: ");

		for(RouteSegmentResult segment :results) {
			...
			if (segment.getRoutingTime() > 0.0f) {
				if(segment.getDistance() > 0.0f) {
					speed = segment.getDistance() / segment.getRoutingTime();
				}
            }

            float actualSpeed = traffic.queryBRASpeed(segment.getObject().getId(),
					lowIntervalValue,
					highIntervalValue
			);
            ...
            if (actualSpeed > ctx.getRouter().getMaxDefaultSpeed()) {
                actualSpeed = ctx.getRouter().getMaxDefaultSpeed();
            }


            segment.setSegmentSpeed(actualSpeed);
            segment.setRoutingTime((float)Math.ceil(segment.getDistance()/actualSpeed));


			highIntervalValue =(long) (Math.ceil(checkpointTime+segment.getRoutingTime())*2);
//			if(previousID == segment.getObject().getId()){
//				checkpointTime++;
//			}

			//System.out.println((long)(checkpointTime + delay) * 2+ " "+(long)(checkpointTime + delay + segment.getRoutingTime()) * 2);

			traffic.addCarToTree(segment.getObject().getId(),
					(long) segment.getDistance(),
					speed,
					lowIntervalValue,
					highIntervalValue
			);
			...
			//long segTraffic = traffic.queryTraffic(segment.getObject().getId(),lowIntervalValue, highIntervalValue) ;
			//CHECK PURPOSES
			//System.out.println("ID: "+segment.getObject().getId()+" | ("+lowIntervalValue+" ; "+highIntervalValue+")"+" TRAFFIC "+segTraffic);


			//if(traffic.queryDensity(segment.getObject().getId(),(long) Math.ceil(checkpointTime  * 2),
			//		(long) Math.ceil( (checkpointTime  + segment.getRoutingTime()) * 2)) ==0 ){
//				System.out.println("ID: "+segment.getObject().getId() +" T: "+
//						traffic.queryTraffic(segment.getObject().getId(),
//								(long) Math.ceil(checkpointTime  * 2),
//								(long) Math.ceil( (checkpointTime  + segment.getRoutingTime()) * 2)) +" D: "+
//						traffic.queryDensity(segment.getObject().getId(),
//								(long) Math.ceil(checkpointTime  * 2),
//								(long) Math.ceil( (checkpointTime  + segment.getRoutingTime()) * 2))
//				);
			//}



//			System.out.println("ID " + segment.getObject().getId() + "  " + actualSpeed+" "+ segment.getDistance()+" "+
//			traffic.queryTraffic(segment.getObject().getId(),
//			 (long) (checkpointTime  ) * 2,
//			 (long) (checkpointTime   + segment.getRoutingTime()) * 2) );

//			System.out.println("ID " + segment.getObject().getId() + "  ( " + (long)checkpointTime+" - "+ (long)(checkpointTime + segment.getRoutingTime())+" ) "+
//					traffic.queryTraffic(segment.getObject().getId(),
//							(long) (checkpointTime  ) * 2,
//							(long) (checkpointTime   + segment.getRoutingTime()) * 2) );

			checkpointTime += segment.getRoutingTime();
		}
		//System.out.println();
	}

	List<RouteSegmentResult> prepareResult(RoutingContext ctx, List<RouteSegmentResult> result, TrafficDataSet traffic) throws IOException {
		validateAllPointsConnected(result);
		splitRoadsAndAttachRoadSegments(ctx, result);
		calculateTimeSpeed(ctx, result, traffic);
	...
	private void calculateTimeSpeed(RoutingContext ctx, List<RouteSegmentResult> result, TrafficDataSet traffic) throws IOException {
		//for Naismith
...
//			float routingTime = ((float)distOnRoadToPass/(float)speed);
//
//			double actualSpeed = (double)traffic.querySpeed((road.getId() >> (BinaryInspector.SHIFT_ID)),
//					(long) Math.ceil(distOnRoadToPass)*2,
//					(long) (Math.ceil(distOnRoadToPass) +Math.ceil(routingTime))*2
//			);
//
//			if(traffic.getAlgorithmUsed().equals("CCRA")){
//				float density = traffic.queryDensity((road.getId() >> (BinaryInspector.SHIFT_ID)),
//						(long) Math.ceil(distOnRoadToPass)*2,
//						(long) (Math.ceil(distOnRoadToPass) +Math.ceil(routingTime))*2
//				);
//
//				if(density >= RouterConstants.TRAFFIC_JAM_DENSITY){
//					if(speed *0.1f < actualSpeed){
//						actualSpeed = speed * 0.1f;
//					}
//				}
//
//			}
...
//
//			if (actualSpeed == 0.0f) {
//				actualSpeed = (ctx.getRouter().getMinDefaultSpeed() * 0.9);
//			}
//
//			// speed can not exceed max default speed according to A*
//			if (actualSpeed> ctx.getRouter().getMaxDefaultSpeed()) {
...
//			speed = actualSpeed;
//
//			distOnRoadToPass = distance/speed;
