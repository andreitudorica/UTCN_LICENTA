...
//		return MapUtils.measuredDist31(x1, y1, x2, y2);
	...
	FinalRouteSegment searchRouteInternal(final RoutingContext ctx, RouteSegmentPoint start, RouteSegmentPoint end,
										  RouteSegment recalculationEnd, TrafficDataSet traffic, long delay) throws InterruptedException, IOException {
		// measure time
		ctx.timeToLoad = 0;
		...
		//Set<Long> visitedSegmentsIDs = new HashSet<Long>();
...
//			System.out.println(segment.getRoad().getId() + " - " + segment.getParentRoute().getRoad().getId());
			...
			if (ctx.memoryOverhead > ctx.config.memoryLimitation * 0.95) {
				System.out.println("USED "+ ctx.memoryOverhead/ (1 << 20));
			...
			if (forwardSearch) {
				...
				processRouteSegment(ctx, false, graphDirectSegments, visitedDirectSegments,
						segment, visitedOppositeSegments, doNotAddIntersections, traffic, delay);
			} else {
				...
				processRouteSegment(ctx, true, graphReverseSegments, visitedOppositeSegments, segment,
						visitedDirectSegments, doNotAddIntersections, traffic, delay);
			}
...
//		} else if (segment.getSegmentStart() == segment.getRoad().getPointsLength() - 1 && positiveDirection && segment.getSegmentStart() > 0) {
	...
	private void processRouteSegment(final RoutingContext ctx, boolean reverseWaySearch,
									 ...
									 TrafficDataSet traffic, long delay) throws IOException {
...
//				ctx.timeNanoToCalcDeviation += (System.nanoTime() - nt);
			...
			float distStartObstacles = segment.distanceFromStart +
                    calculateTimeWithObstaclesAndTraffic(ctx, road,segmentDist, segment.getDistanceFromStart() + delay, obstaclesTime, traffic);
...
//		if(!visitedSegmentsIDs.contains(Long.valueOf(road.getId() >> (BinaryInspector.SHIFT_ID)))) {
//			float priority = ctx.getRouter().defineSpeedPriority(road);
//			float speed = (ctx.getRouter().defineRoutingSpeed(road) * priority);
//			float routingTime = segmentDist / speed;
//
//			//add car to traffic
//			traffic.addCarToTree((road.getId() >> (BinaryInspector.SHIFT_ID)),
//					(long) Math.ceil(segmentDist),
//					speed,
//					(long) Math.ceil(segment.getDistanceFromStart()) * 2,
//					(long) (Math.ceil(segment.getDistanceFromStart()) + Math.ceil(routingTime)) * 2
//			);
//			visitedSegmentsIDs.add(Long.valueOf(road.getId() >> (BinaryInspector.SHIFT_ID)));
//		}
	}
	...
	private double getSegmentDistance(RouteDataObject road, int startPointIndex, int endPointIndex ){
		int next;
		boolean plus = startPointIndex < endPointIndex;

		double distance = 0;

		for (int j = startPointIndex; j != endPointIndex; j = next) {
			next = plus ? j + 1 : j - 1;

			double d = MapUtils.getDistance(
					MapUtils.get31LatitudeY(road.getPoint31XTile(j)),
					MapUtils.get31LongitudeX(road.getPoint31YTile(j)),
					MapUtils.get31LatitudeY(road.getPoint31XTile(next)),
					MapUtils.get31LongitudeX(road.getPoint31YTile(next)));
	...
	private float calculateTimeWithObstaclesAndTraffic(RoutingContext ctx, RouteDataObject road, float distOnRoadToPass, float timeFromStart, float obstaclesTime, TrafficDataSet traffic){
		float priority = ctx.getRouter().defineSpeedPriority(road);
		float speed = (ctx.getRouter().defineRoutingSpeed(road) * priority);
		float routingTime = distOnRoadToPass/ speed;

		//long segmentId = (road.getId()>> (BinaryInspector.SHIFT_ID));
		//schimb in interval tree sa pun paramentru speed care il iau de mai sus
		//reformulez si pun actual speed in conditions sa fie in intervalul min max definit de contextul de mai jos
		//partea de threshold e prezenta in get traffic
		//in cazul in care aplica CCRA trebuie verificat densitatea, iar daca e mai mare sau egala cu densitatea de threshold


		float actualSpeed = traffic.querySpeed(road.getId(),
				(long) Math.ceil(timeFromStart)*2,
				(long) (Math.ceil(timeFromStart) +Math.ceil(routingTime))*2
		);

//		if(traffic.getAlgorithmUsed().equals("CCRA")){
//			float density = traffic.queryDensity(segmentId ,
//					(long) Math.ceil(timeFromStart)*2,
//					(long) (Math.ceil(timeFromStart) +Math.ceil(routingTime))*2
//			);
//
...
//		}
		...
		if (actualSpeed == 0.0f) {
			actualSpeed = (ctx.getRouter().getMinDefaultSpeed() * priority);
		}
		...
		return obstaclesTime + distOnRoadToPass / actualSpeed;
	}
	...
	private boolean processRestrictions(RoutingContext ctx, RouteSegment segment, RouteSegment inputNext, boolean reverseWay) {
	...
	private RouteSegment processIntersections(RoutingContext ctx, PriorityQueue<RouteSegment> graphSegments,
		...
		if (inputNext != null && inputNext.getRoad().getId() == segment.getRoad().getId() && inputNext.next == null) {
			...
			thereAreRestrictions = processRestrictions(ctx, segment, inputNext, reverseWaySearch);
